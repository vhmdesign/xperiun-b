...
/* â”€â”€ Helper: em viewport â‰¤576px (mobile portrait), neutraliza blurs de
   animaÃ§Ã£o. Devolve 'none' quando estreito; senÃ£o devolve 'blur(Xpx)'.
   AnimaÃ§Ãµes de opacity/transform continuam funcionando â€” sÃ³ o blur some. */
...
/* â”€â”€ sec-depos: avatares aleatÃ³rios â”€â”€ */
(function () {
    var thumbs = [
...
/* â”€â”€ sec-depos: rotaÃ§Ã£o dos logos â”€â”€ */
(function () {
    var logos = [
    ...
    var imgToLogo = new Map();  // rastreia qual logo estÃ¡ em cada slot â€” garante unicidade
    ...
    function tick() {
        ...
        /* Pool: todos os logos que NÃƒO estÃ£o atualmente em algum slot visÃ­vel.
           Garante que nunca haverÃ¡ logos repetidos ao mesmo tempo. */
        var displayed = displayedLogos();
        var pool = logos.filter(function(l) { return displayed.indexOf(l) === -1; });
        if (!pool.length) return;
        var next = pool[Math.floor(Math.random() * pool.length)];

        imgToLogo.set(img, next);  /* atualiza ANTES do preload pra prÃ³ximos ticks jÃ¡ enxergarem */
        img.style.opacity = '0';
        /* Swap sÃ³ dispara quando AMBOS aconteceram:
           1. fade-out terminou (600ms, casando com transition CSS)
           2. prÃ³ximo logo carregou (preload via new Image)
           Evita flicker e mantÃ©m transiÃ§Ã£o lenta e suave. */
...
/* â”€â”€ sec-depos: carrossel (loop infinito) + facade de vÃ­deo â”€â”€ */
(function () {
...
/* â”€â”€ sec-perfis: accordion (clique em todo o .perfis-row) â”€â”€ */
(function () {
...
/* â”€â”€ sec-faq: accordion (clique em todo o .faq-row) â”€â”€ */
(function () {
...
/* â”€â”€ Mini-carrossel da sec-oferta (apenas ativo abaixo de 864px via CSS) â”€â”€ */
(function () {
...
/* sec-oferta / sec-anuidade / sec-faq: comportamento UNIFICADO entre
   ...
   evitar a seÃ§Ã£o sumir ao alternar viewport e pra eliminar a animaÃ§Ã£o
   de translate. Tudo Ã© controlado por CSS sticky + z-index agora. */

/* â”€â”€ --anuidade-vh: 1% do viewport visÃ­vel REAL em pixels â”€â”€
   Atualizado em tempo real via visualViewport.resize (sem o delay do `dvh`
   no Chrome Android quando a barra de endereÃ§o aparece/some). Usado pelo CSS
   pra calcular position/height de sec-oferta/sec-anuidade/anuidade-sticky-wrap
   sem depender do `vh` "large viewport" estÃ¡tico do browser.
   Roda em DESKTOP e MOBILE â€” desktop tambÃ©m pode ter mudanÃ§as de viewport
...
/* â”€â”€ AnimaÃ§Ã£o scroll-driven dos elementos de sec-anuidade â”€â”€
   ...
   Fases (scrolled = -wrap.getBoundingClientRect().top, normalizada por vh):
     Â· 0    â†’ 100vh : fade-in   (opacity 0 â†’ 1, blur 16 â†’ 0, translate â†’ 0)
     Â· 100  â†’ 200vh : hold      (estÃ¡vel)
     Â· 200  â†’ 300vh : fade-out  (reverte tudo)
   Title Ã© animado char-by-char com stagger: cada letra tem sua janela de
   animaÃ§Ã£o dentro da fase de fade-in/out. Eyebrow/desc/actions usam
...
(function () {
    ...
    /* Split do tÃ­tulo em char-spans. Estrutura criada:
         <h2 class="anuidade-title">            â† recebe filter:blur (sem mask)
           <span class="anuidade-title-mask">   â† tem mask-image (sem filter)
             [wordWraps com char spans dentro]
           </span>
         </h2>
       A separaÃ§Ã£o evita que a mask clippe o halo do blur. char spans sÃ³ animam
    ...
    if (title && !title.dataset.split) {
        ...
        (function walk(node) {
            if (node.nodeType === 3) {
                ...
                /* RecursÃ£o em element nodes (ex: <span style="font-weight:700">,
    ...
    /* Stagger config: cada char comeÃ§a 10ms "apÃ³s" o anterior (em tempo
       fictÃ­cio). Char anima por 300ms. Total = (N-1)*10 + 300. A fraÃ§Ã£o
    ...
    function tick() {
        ...
        /* Toggle .is-active no section enquanto sticky estÃ¡ engajado.
           menu.js observa essa classe e recolhe o nav. Guard idempotente
           pra nÃ£o disparar MutationObserver toda frame. */
        ...
        /* Title: filter uniforme no elemento externo (pra nÃ£o ser clippado
           pela mask que vive no .anuidade-title-mask interno). Char spans
           sÃ³ animam opacity + translate. */
        if (title) {
...
/* â”€â”€ Reveal: sec-anuidade sobe sobre sec-oferta (mobile â‰¤864px) â”€â”€
   sec-oferta Ã© position: sticky; top: calc(100vh - var(--sec-oferta-height)),
   mesmo padrÃ£o de sec-projetos. CSS faz o pin. Esse IIFE sÃ³ mantÃ©m
...
/* O botÃ£o "Garantir minha vaga" (a[href="#sec-oferta"]) agora usa o
   atributo data-scroll-to="end" â€” scroll-suave.js cuida do destino
   sticky-engage (bottom de sec-oferta alinhado com bottom do viewport,
   mostrando os cards). LÃ³gica centralizada lÃ¡ pra evitar briga entre
   meu handler aqui e o tick() do scroll-suave. */

/* â”€â”€ Reveal: pilares-profs sobe sobre sec-projetos â”€â”€
   sec-projetos Ã© position: sticky; top: calc(100vh - var(--sec-projetos-height)).
   ...
   offset negativo. Esse IIFE atualiza --sec-projetos-height em tempo real:
    - ResizeObserver: dispara quando sec-projetos muda de tamanho (mudanÃ§a de
      conteÃºdo, font load, breakpoint, etc.)
    - visualViewport.resize / window.resize: pra mudanÃ§as de vh (barra de
      endereÃ§o no mobile, rotaÃ§Ã£o, redimensionamento de janela) â€” quando o vh
      muda, elementos dentro de sec-projetos que usam vh recalculam altura, o
      RO pega automaticamente, mas escutar aqui tambÃ©m garante atualizaÃ§Ã£o
      mesmo se a altura de sec nÃ£o mudar (caso raro). */
(function () {
...
/* â”€â”€ Reveal animation: sec-eyebrow + sec-title (char-by-char) + sec-lede â”€â”€
   Imita o padrÃ£o de home.html. CSS jÃ¡ define o estado inicial (opacity 0 +
   blur + translate). JS prepara o tÃ­tulo quebrado em chars (cada char em
...
(function () {
    ...
    /* Prepara todos os tÃ­tulos no load pra evitar flash de texto sem spans
       Inclui hero-h1 (animado separadamente no load, nÃ£o via observer). */
    document.querySelectorAll('.sec-title, .hero-h1').forEach(prepareTitle);

    /* â”€â”€ Hero entry: dispara imediatamente no load (above the fold) â”€â”€ */
    function animateHero() {
        ...
        /* hero-actions buttons: mesma animaÃ§Ã£o de .completa-tile em home.html
           (cta-card-enter, 0.6s). Stagger de 100ms entre eles, apÃ³s o lede. */
    ...
    /* Espera 2 frames pra garantir que o estado inicial CSS foi pintado antes
       de aplicar os estilos finais (caso contrÃ¡rio, transiÃ§Ã£o nÃ£o dispara). */
    requestAnimationFrame(function () {
        requestAnimationFrame(animateHero);
    });

    /* Observer: dispara quando o tÃ­tulo entra na viewport (com margem -25% bottom
       pra esperar o usuÃ¡rio rolar atÃ© a Ã¡rea visÃ­vel antes da animaÃ§Ã£o) */
    ...
    document.querySelectorAll('.sec-title').forEach(function (title) {
        ...
        /* lede pode estar fora do parent imediato (ex: .cta-sub Ã© irmÃ£o do .cta-header).
           Procura no parent primeiro; se nÃ£o achar, sobe um nÃ­vel. */
    ...
    /* meta-card: entrada com fade/blur/translate + glow contÃ­nuo. Dispara
       na carga da pÃ¡gina (nÃ£o no scroll/intersection), porque o card vive
       no fold do hero â€” esperar scroll nÃ£o fazia sentido. Stagger de 80ms
       entre mÃºltiplos cards. Dois rAFs garantem que o estado inicial CSS
       foi pintado antes do .is-entered (sem isso, transition nÃ£o dispara). */
    requestAnimationFrame(function () {
        requestAnimationFrame(function () {
    ...
    /* skill-tile, course-card, etc: mesma animaÃ§Ã£o de .completa-tile em home.html
       (cta-card-enter, 0.6s). Stagger de 40ms entre elementos.
       Stagger usa ordem de intersecÃ§Ã£o (nÃ£o DOM) â€” assim cards fora do viewport
       inicial (ex: clones do depos-carousel) nÃ£o consomem delay dos visÃ­veis. */
    ...
    /* Observer Ãºnico pra containers (animam como um bloco) */
    ...
    /* sec-stats: dispara na carga (nÃ£o no scroll), mesmo padrÃ£o do .meta-card
       â€” vive no fold do hub e nÃ£o fazia sentido esperar intersection. */
    requestAnimationFrame(function () {
        requestAnimationFrame(function () {
    ...
    /* depos-header: anima depos-social-proof (vindo de cima) e depos-stars
       ...
       porque esses elementos nÃ£o sÃ£o cobertos por animateHeader. */
